<template>
  <form @submit.prevent="onSubmit">
    <div class="form-control" :class="{ invalid: fError }">
      ...
      <input id="fio" type="text" @blur="fBlur" v-model="fio" />
      <small v-if="fError">{{ fError }}</small>
    </div>
    <div class="form-control" :class="{ invalid: pError }">
      <label for="phone">Tелефон</label>
      <input id="phone" type="text" @blur="pBlur" v-model="phone" />
      <small v-if="pError">{{ pError }}</small>
    </div>
    <div class="form-control" :class="{ invalid: aError }">
      <label for="amount">Суммы</label>
      <input id="amount" type="number" @blur="aBlur" v-model.number="amount" />
...
export default {
  ...
  setup(_, { emit }) {
    ...
    const submit = async (values) => {
      await store.dispatch('requestModule/create', values)
      console.log(values);
      emit('created')
    };
